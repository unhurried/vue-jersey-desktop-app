...
public class CorsFilter implements ContainerResponseFilter {
	...
	public void filter(ContainerRequestContext request, ContainerResponseContext response) throws IOException {
		if (Arrays.asList(environment.getActiveProfiles()).contains("development")) {
			...
			response.getHeaders().put("Access-Control-Allow-Headers", Arrays.asList("origin, content-type, accept, authorization"));
			response.getHeaders().put("Access-Control-Allow-Credentials", Arrays.asList("true"));
			response.getHeaders().put("Access-Control-Allow-Methods", Arrays.asList("GET, POST, PUT, DELETE, OPTIONS, HEAD"));
		}
	}
}
